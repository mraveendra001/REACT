...
const Header = () => {
  ...
  const cartItems = useSelector(store => store.cart.items); //subscribing to store .cart.items
  // we are only subscribing to cart items , it may other slices too
 ...
 const {user}= useContext(UserContext);//usercontext will get the user from usecontext
  ...
  <ul className="flex py-10 ">
      ...
      <li className="px-2 text-white">Cart - {cartItems.length} items</li>
  </ul>
  </div>
  <h1>{isOnline ? "✅" : "🔴"}</h1>
